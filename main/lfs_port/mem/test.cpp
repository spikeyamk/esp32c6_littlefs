#include <cstdint>
#include <cstring>
#include <cstdio>
#include <cassert>
#include <vector>
#include <array>

#include <littlefs/lfs.h>

#include "lfs_port/mem/test.hpp"

namespace LFS {
    namespace Mem {
        int read(const struct lfs_config *c, lfs_block_t block, lfs_off_t off, void *buffer, lfs_size_t size) {
            uint8_t* storage = static_cast<uint8_t*>(c->context);
            std::memcpy(buffer, (storage + (block * c->block_size)) + off, size);
            return LFS_ERR_OK;
        }

        int erase(const struct lfs_config *c, lfs_block_t block) {
            uint8_t* storage = static_cast<uint8_t*>(c->context);
            std::memset(storage + (block * c->block_size), 0, c->block_size);
            return LFS_ERR_OK;
        }

        int prog(const struct lfs_config *c, lfs_block_t block, lfs_off_t off, const void *buffer, lfs_size_t size) {
            uint8_t* storage = static_cast<uint8_t*>(c->context);
            std::memcpy((storage + (block * c->block_size)) + off, buffer, size);
            return LFS_ERR_OK;
        }

        int sync(const struct lfs_config *c) {
            (void) c;
            return LFS_ERR_OK;
        }

        int test() {
            lfs_t lfs;

            const lfs_size_t block_size { 512 };
            const lfs_size_t block_count { 8 };
            static uint8_t read_buffer[block_size];
            static uint8_t prog_buffer[block_size];
            static uint8_t lookahead_buffer[block_size];
            static uint8_t storage[block_count * block_size];

            const lfs_config config {
                .context = static_cast<void*>(storage),
                .read = read,
                .prog = prog,
                .erase = erase,
                .sync = sync,
            #ifdef LFS_THREADSAFE
                .lock = nullptr,
                .unlock = nullptr,
            #endif
                .read_size = block_size,
                .prog_size = block_size,
                .block_size = block_size,
                .block_count = block_count,
                .block_cycles = -1,
                .cache_size = block_size,
                .lookahead_size = block_size,
                .compact_thresh = static_cast<lfs_size_t>(-1),

                .read_buffer = static_cast<void*>(read_buffer),
                .prog_buffer = static_cast<void*>(prog_buffer),
                .lookahead_buffer = static_cast<void*>(lookahead_buffer),

                .name_max = 0,
                .file_max = 0,
                .attr_max = 0,
                .metadata_max = 0,
                .inline_max = static_cast<lfs_size_t>(-1),
            };

            lfs_format(&lfs, &config);

            // mount the filesystem
            lfs_mount(&lfs, &config);
            
            lfs_file_t file;

            lfs_file_open(&lfs, &file, "boot_count", LFS_O_RDWR | LFS_O_CREAT);
            static const std::array<uint8_t, 600> message { 
                ,
            };
            lfs_file_write(&lfs, &file, static_cast<const void*>(message.data()), message.size());
            lfs_file_close(&lfs, &file);

            lfs_file_open(&lfs, &file, "boot_count", LFS_O_RDWR | LFS_O_CREAT);
            static std::array<uint8_t, message.size()> read_message;
            lfs_file_read(&lfs, &file, static_cast<void*>(read_message.data()), read_message.size());
            lfs_file_close(&lfs, &file);

            if(read_message != message) {
                return -1;
            }

            lfs_unmount(&lfs);
            return 0;
        }
    }
}
